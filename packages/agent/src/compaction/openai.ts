...
export function shouldUseOpenAiRemoteCompaction(model: Model): boolean {
	...
	// Codex always uses remote compaction.
	if (model.provider === "openai-codex") return true;
	if (model.provider === "openai") {
		// Explicit opt-in overrides the host restriction; otherwise only the real
		// first-party OpenAI API (not a custom gateway/proxy fronting `provider:
		// openai`) gets remote compaction by default.
		return model.remoteCompaction?.enabled === true || isOpenAiApiBaseUrl(model.baseUrl);
	}
	// Other providers must explicitly enable remote compaction and target a
	// Responses-family API.

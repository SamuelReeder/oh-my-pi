...
const OVERFLOW_NO_BODY_413_PATTERN = /\b413\s*(status code)?\s*\(no body\)/i;
// OpenAI Responses gateways also use 400/no-body for schema/history validation
// failures, so 400/no-body is only treated as overflow off those APIs.
const OVERFLOW_NO_BODY_400_PATTERN = /\b400\s*(status code)?\s*\(no body\)/i;
...
function matchesOverflowText(text: string, api?: Api): boolean {
	if (OVERFLOW_PATTERNS.some(p => p.test(text)) || OVERFLOW_NO_BODY_413_PATTERN.test(text)) return true;
	const noBody400IsOverflow = api !== "openai-responses" && api !== "openai-codex-responses";
	return noBody400IsOverflow && OVERFLOW_NO_BODY_400_PATTERN.test(text);
}

function classifyText(errorMessage: string | undefined, errorStatus: number | undefined, api?: Api): number {
	let kinds = 0;
	if (errorMessage) {
		if (matchesOverflowText(errorMessage, api)) kinds |= Flag.ContextOverflow;
...
export function isContextOverflow(message: AssistantMessage, contextWindow?: number): boolean {
	...
	return (
		message.stopReason === "error" && !!message.errorMessage && matchesOverflowText(message.errorMessage, message.api)
	);
}

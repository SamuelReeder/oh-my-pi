...
export function buildAnthropicCompat(spec: ModelSpec<"anthropic-messages">): ResolvedAnthropicCompat {
	...
	// The canonical `anthropic` provider is treated as a signing endpoint even
	// when its baseUrl is overridden to a corporate gateway (e.g. AMD's
	// `llm-api.amd.com/Anthropic`): it still fronts the real Anthropic API,
	// which rejects unsigned/empty-signature thinking.
	const signingEndpoint =
		official || isCopilot || isZenmux || isAnthropicSigningProxyUrl(baseUrl) || spec.provider === "anthropic";
	const compat: ResolvedAnthropicCompat = {
		...
		// id or baseUrl marker. The canonical `anthropic` provider is folded into
		// `signingEndpoint` above even when its baseUrl is overridden to a
		// corporate gateway (e.g. AMD's `llm-api.amd.com/Anthropic`): it still
		// fronts the real Anthropic API, which rejects unsigned thinking, and
		// `transform-messages` already treats `provider === "anthropic"` as
		// official-involved and strips foreign signatures on a mid-conversation
		// model switch (#2257-followup).
